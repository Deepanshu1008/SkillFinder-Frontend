...
const Profile = () => {
    ...
    const userData = async () => {
      try {
        const res = await fetch(
          'http://127.0.0.1:8000/fetchuserprofile',
          {
            headers: {
        ...
        if (res.status === 200) {
          ...
          setEmail(data['email']);
          setCollg('Jaypee University OF Engineering and Technology');
          setBatch(data['batch']);
          setBio(data['bio']);
          setGithub(
            'socials' in data
  ...
  useEffect(() => {
    const uploadImage = async () => {
      ...
      if (image) {
        ...
        try {
          formData.append('upload_preset', 'SkillFinder');
          ...
          if (res.status === 200) {
            ...
            try {
              res = await fetch(
                'http://127.0.0.1:8000/updateuserpic',
                {
                  method: 'PUT',
                  headers: {
                    'Content-Type': 'application/json',
  ...
  useEffect(() => {
    const queryData = async () => {
      setloading(true);
      const res = await fetch(
        `http://127.0.0.1:8000/skillssuggestions?q=${query}`
      );
      if (res.status === 200) {
        const Data = await res.json();
        let filteredData = [
  ...
  const updateUserData = async () => {
    try {
      ...
      const res = await fetch(
        'http://127.0.0.1:8000/updateuserprofile',
        {
          method: 'PUT',
          headers: {
            'Content-Type': 'application/json',
