...
const Messages = () => {
  ...
  const fetchSuggestions = async (search = "",skip = 0) => {
    ...
    if(search === "" && fetchBool)
    {
      res = await fetch(`http://127.0.0.1:8000/users/data?skip=${skip}`, {
        headers:{
    ...
    {
      res = await fetch(`http://127.0.0.1:8000/searchmessage?q=${search}`,{
        headers:{
          authentication: `Bearer ${token}`
        }
      })
  ...
  const UpdateMessagesUserData = async (id) => {
    setLoading(true)
    let userData = {}
    let data = await fetch("http://127.0.0.1:8000/auth/getUser", {
      method: 'POST',
      headers: {
